...
class Tool {
    ...
    /**
     * Get HTML template for the tab content
     * Should return the Vue template HTML for this tool's content
     * @returns {String} HTML string for the tab content
     */
    getTabContentHTML() {
        return `
            <div v-if="activeTab === '${this.id}'" class="tab-content">
                <p>Tool content for ${this.name}</p>
            </div>
        `;
    }
